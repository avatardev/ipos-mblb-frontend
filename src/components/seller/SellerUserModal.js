...
export default function SellerUserModal({showSellerUserModal, setShowSellerUserModal, setIdUserSeller, idUserSeller, corpId, setChanges}) {
    ...
    useEffect(() => {
      if (idUserSeller) {
        fetchData(`/user/sellers/${idUserSeller}`)
        .then(res => {
    ...
    const handleSubmitSellerUser = () => {
        if (password === confirmPassword) {
          const body = {
            username,
            password,
            seller_id: parseInt(corpId)
          }
          console.log(body);
          if (!idUserSeller) {
            postData('/user/sellers', body)
          .then(setChanges(current => current + 1))
          } else {
            putData(`/user/sellers/${idUserSeller}`, body)
            .then(setChanges(current => current + 1))
          }
          handleClearInput();
                ...
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                  ...
                  <div className="flex items-center justify-end p-3 border-t border-solid border-gray rounded-b">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={handleSubmitSellerUser}
                    >
                      Simpan
                    </button>
                  </div>
