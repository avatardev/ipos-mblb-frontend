...
const CheckerUser = () => {
  ...
  const { data, isLoading, error } = useFetch(
    `/user/checkers?offset=${offset}&limit=${limit}&keyword=${keyword}`,
    changes
  );

  const handleDelete = () => {
    deleteData(`/user/checkers/${itemId}`).then((res) => {
      console.log(res);
        ...
        <div className="sm:px-[32px] pb-[32px] w-full">
          ...
          <div className="bg-white h-fit px-3 overflow-x-auto">
            <div className="flex justify-end gap-5 py-3">
              <button
                onClick={() => setShowCheckerUserModal(true)}
                              ...
                              >
                                ...
                                <td className="py-2 px-6 text-md text-gray-500 whitespace-nowrap dark:text-gray-400">
                                  <div className="flex gap-3">
                                    <button
                                      onClick={() => {
                                        setIdUserChecker(item.id);
                                        setShowCheckerUserModal(true);
                                      }}
