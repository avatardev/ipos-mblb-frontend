...
const BuyerUser = () => {
  ...
  const { data, isLoading, error } = useFetch(
    `/user/buyers?v_plate=${corpId}&offset=${offset}&limit=${limit}&keyword=${keyword}`,
    changes
  );

  const handleDelete = () => {
    deleteData(`/user/buyers/${itemId}`).then((res) => {
      console.log(res);
        ...
        <div className="sm:px-[32px] pb-[32px] w-full">
          ...
          <div className="shadow-lg rounded-lg bg-white h-fit px-3 py-2">
            <div className="flex justify-end gap-5 py-3">
              <button
                onClick={() => setShowBuyerUserModal(true)}
                        ...
                        <tbody>
                          {error ? (
                            ...
                            data?.user.map((item, i) => (
                              ...
                              >
                                <td className="py-2 px-6 text-md  text-gray-900 whitespace-nowrap">
                                  {i + offset + 1}
                                </td>
                                ...
                                <td className="py-2 px-6 text-md text-gray-500 whitespace-nowrap dark:text-gray-400">
                                  <div className="flex gap-3">
                                    <button
                                      onClick={() => {
                                        setIdUserBuyer(item.id);
                                        setShowBuyerUserModal(true);
                                      }}
